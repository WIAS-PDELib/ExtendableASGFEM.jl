...
function main()
    ...
    for T in (Float64, Double64)
        if normalize
            tolerance = T == Float64 ? 1.0e-12 : 1.0e-12
        else
            tolerance = T == Float64 ? 1.0e-8 : 1.0e-19
        end
        val::T = 0.0
        val_compare::T = 0.0
        ...
        for j in 0:order, k in j:order, l in k:order
    ...
    @info "Checking triple products..."
    for j in 1:nmi, k in 1:nmi, l in 1:nmi
