...
finite element solution of the determinstic model problem with fixed (sampled) coefficient with
...
function calculate_sampling_error(
    ...
    for s = 1 : nsamples
        ...
        if debug ## plot determinstic solution and evaluation of SGFEM solution for sample
    ...
    for m = 1 : length(totalerrorL2stress_weighted)
