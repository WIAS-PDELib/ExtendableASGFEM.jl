...
function show_results(; force = false, mode_history_up_to_level = 15, kwargs...)
    ...
    istart = 1
    for j = 1 : min(mode_history_up_to_level, nlevels)
...
function main(data = nothing; 
    ...
    df = DataFrame(ndofs_space=zeros(Int, 0),
        nmodes=zeros(Int, 0),
    ...
    while (true)
        ...
        ansatz_deg = maximum([maximum(multi_indices[k]) for k = 1 : length(multi_indices)])+4
        ...
        for j = 1 : length(multi_indices_extended)
        ...
        df[lvl, :ndofs_space] = sum([sol.FES_space[j].ndofs for j = 1 : length(sol.FES_space)])
...
function produce_plots(; 
    ...
    datas = Array{Dict{String,Any},2}(undef, length(order), length(decay))
    for o = 1 : length(order), d = 1 : length(decay)
        data["order"] = order[o]
        data["decay"] = decay[d]
        datas[o,d], ~ = produce_or_load(main, data, filename = filename, force = force)
        xgrid = datas[o,d]["solution"].FES_space[1].xgrid
        repair_grid!(xgrid)
    ...
    for d = 1 : length(decay)
        ...
        if xscale !== nothing
            ax_conv = Axis(f_conv[1,1], title = title, xlabel = xlabel, ylabel = ylabel, yscale = yscale, xscale = xscale,
            ...
            ax_conv = Axis(f_conv[1,1], title = title, xlabel = xlabel, ylabel = ylabel, yscale = yscale,
            xminorticksvisible = true, xminorgridvisible = true, yminorgridvisible = true, yminorticksvisible = true,
            xminorticks = IntervalsBetween(10), yminorticks = IntervalsBetween(10))
        end
    ...
    for o = 1 : length(order)
        ...
        for d = 1 : length(decay)
            ...
            max4mode[d,1:M] = [maximum([multi_indices[j][k] for j = 1 : nmodes]) for k = 1 : M]
        ...
        yticks = (1:maxdegree, ["$(Int(j))" for j = 1 : maxdegree])
        xticks = (1:plotM, [j % 2 == 1 ? "$(Int(j))" : "" for j = 1 : plotM])
